...
void drawing(Mat frame){
	vector<Pt> bodys = worms.getBody();
	for(int i=bodys.size()-1;i>=0;i--){
		if(i==0)	circle(frame, Point(bodys[i].x, bodys[i].y), OBJECT_SIZE, Scalar(255, 255, 255), -1);
		else
			circle(frame, Point(bodys[i].x, bodys[i].y), OBJECT_SIZE, Scalar(255, 0, 0), -1);
